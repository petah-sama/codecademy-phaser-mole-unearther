// track amount of points won
let pointsWon = 0;

// time the user took to hit the mole
let reactionTime = 0;
...
class GameScene extends Phaser.Scene {
  ...
  update() {
    ...
    const applyHitReward = () => {
      // give the user points according to how long he took to hit the mole
      if (reactionTime < 0.8) {
        updateScore(10);
        reactionTime = 0;
        pointsWon = 10;
      } else if (reactionTime >= 0.8 && reactionTime < 1.2) {
        updateScore(7);
        reactionTime = 0;
        pointsWon = 7;
      } else if (reactionTime >= 1.2 && reactionTime < 1.6) {
        updateScore(5);
        reactionTime = 0;
        pointsWon = 5;
      } else if (reactionTime >= 1.6 && reactionTime < 2) {
        updateScore(3);
        reactionTime = 0;
        pointsWon = 3;
      } else {
    ...
    // check the key the user is hitting
  ...
  initializeTimer(timerCallback) {
    ...
    this.time.addEvent({
      delay: 100, // call after every 100ms (0.1sec)
      callback: timerCallback, // run function after 0.1sec elapsed
  ...
  updateTimerText() {
    gameState.timerText.setText(`Time: ${Math.floor(timeLeft)}`);
  }
  ...
  displayRewardText() {
    ...
    const rewardText = this.add.text(160, 50, `+${pointsWon}`).setColor('#228B22');
    this.time.addEvent({
